...
export const useProjectStore = create<ProjectState>((set, get) => ({
  ...
  createProject: async (name, description, model) => {
    ...
    try {
      ...
      console.error('Failed to create project in localStorage:', e);
    }
    return null;
  },
  ...
  compareVersions: async (v1, v2) => {
    ...
    try {
      ...
      if (stored1 && stored2) {
        const m1 = JSON.parse(stored1);
        const m2 = JSON.parse(stored2);
        const diffs = computeModelDiff(m1, m2);
        set({ diffResult: diffs, diffVersions: [v1, v2] });
      }
    } catch (e) {
      console.error('Failed to compare versions in localStorage:', e);
    } finally {
      set({ isLoading: false });
    }
  },
